# FrameNest test-NUC routine immutable release deploy (f5fbdce), retry after sudo
...
f5fbdce…, or performed Worker 26, stop BLOCKED.
```text
...
Recommendation basis: live sudo/systemd/release/backup/schema/rollback on Michal's test NUC after Worker 26 blocked on invalid remote sudo -n
...
Worker 25 observed leftover lock/43c9849… removed and live 148b6c2… (stale until re-read). Worker 26 observed BatchMode SSH OK and remote sudo -n true exit 1; status/check/deploy were not run. Re-read from framenest-release status. Do not forge .framenest-release-manifest.json on the immutable current tree. If the lock dir, 43c9849…, 43c9849….staging, or f5fbdce….staging is present before deploy, stop BLOCKED; do not delete.
...
Mandatory reading
AGENTS.md, WORKER_EXECUTION_CONTRACT.md, UBUNTU_NUC_DEPLOYMENT.md (Routine Immutable Release Update), ADR-0060, .ap/AP.md, .ap/AP_WORKER.md, Worker 21 as prior failed-deploy claim, Worker 25 as leftover-recovery claim only, Worker 26 as sudo-timestamp failure claim only.
