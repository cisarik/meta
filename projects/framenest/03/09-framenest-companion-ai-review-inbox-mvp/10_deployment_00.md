...
Worker 09 must already have published `6e20fc12` to public `main`. The
COOPERATOR wants that SHA live on the Ubuntu NUC so he can test the
administrator companion review inbox. ADR-0060 is **same-schema only**. This
candidate packages Alembic **0031**; public `main` before publication still
packaged only through **0029**. Expect a schema jump. Do **not** pretend a
single `deploy --yes` will cut over.
...
1. Worker 09 `publication-PASS` and credential-free `ls-remote` equals
   `6e20fc12f145286e474294b79cbd120df6e38e56`.
2. The COOPERATOR has established the remote sudo timestamp **outside** this
   chat (`sudo -v`, then `sudo -n true` on the NUC). You must not run
   `sudo -v` or handle a password.
...
Prior publication report: Worker 09 / exchange 01 publication-PASS required
...
Downloadable prompt filename: 10_deployment_00.md
Destination path: projects/framenest/03/09-framenest-companion-ai-review-inbox-mvp/10_deployment_00.md
...
Frozen hash at issuance (re-check `09_publication_00.md`; stop `BLOCKED` if
drifted):
...
is already present **before** step 6, stop `BLOCKED`. Do not delete a lock you
did not create in this session unless status proves current is unchanged and
the lock is exactly `/run/framenest-release-deploy` from a prior failed
same-SHA prepare; if that is the case, report the finding and wait — do not
guess.
