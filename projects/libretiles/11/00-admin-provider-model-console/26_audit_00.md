...
⛔ You are the auditor. Your first actions are the repository gate, then the required pytest in §1, then source review of C1–C12. A confirmation that a prior grant was consistent is **not** this task. Do not write “the next step is a fresh audit.” Do not stop after agreeing that F-V/F-W are the only `_migration_process` callers. That fact is already established. Produce the security audit report.
...
Prior attempts at this 7-IA did not produce product evidence:

```text
Session 24: BLOCKED before pytest because the required four-module command would have executed
  two historical migration tests whose helper sets PYTHON_DOTENV_DISABLED in a subprocess.
  Orchestration grant conflict. Finding APMC-S7-IA-F01 (info) accepted. Not a product defect.
Session 25: received a prompt that already deselected those two tests, then returned a five-line
  confirmation and ran no tests. That text is not an audit report. Do not copy it. Do not treat
  “C1–C12 remain unaccepted pending evidence” as your verdict table.
```

This prompt is the grant. The deselects in §1 already resolve F01. Execute.
...
The implementer's report (`23_report_00.md`) is a CLAIM. Re-measure. Enumeration status: hypothesis.
Do not treat Orchestrator notes or implementer F01–F11 names as proof.
`24_report_00.md` is authorized reading **only** as the orchestration-blocker record (APMC-S7-IA-F01).
`25_report_00.md` is a non-conforming confirmation: not a finding source, not a verdict table.

Prior findings — do not ignore, do not silently reopen closed ones:

```text
APMC-S7-IA-F01 (session 24, confirmed, info, Orchestrator-accepted): required four-module
  pytest would execute `_migration_process` (test_diagnostic_session.py:612) which sets
  PYTHON_DOTENV_DISABLED in a subprocess. Not a product C/I/A impact. THIS prompt deselects
  those two tests. ⛔ Do not BLOCK because the helper still exists in source.
  Product findings in THIS report start at APMC-S7-IA-F02. Do not re-issue F01.
...
`PYTHON_DOTENV_DISABLED` in the process environment:
...
Focused dynamic evidence (required). Run this exact command from `backend/` **in this session**.
The two `--deselect` nodeids are historical slice-4 migration-rescue tests (F-V / F-W). They are
**not** C1–C12 evidence. They are the only callers of `_migration_process`. Collecting
`test_diagnostic_session.py` is authorized; **executing** those two nodeids is not required
and is not part of this grant.
...
Static helper check (required, not a stop): confirm `_migration_process` has no callers
other than those two deselected tests. If you find another caller, that is a finding
(F02+), not a reason to refuse the required pytest. Importing the helper during collection
does not set `PYTHON_DOTENV_DISABLED`; only executing the helper does.

AP.md:2466 “validation requires a forbidden command” does **not** fire on: (a) the helper’s
source existing, (b) collection of `test_diagnostic_session.py`, or (c) this deselected
command. It **would** fire if you prefixed your own mypy/ruff/pytest with
`PYTHON_DOTENV_DISABLED=1`, or if you removed the deselects and executed F-V/F-W. Do neither.

You MAY add your own throwaway Django/Node probes in test transactions. Full-suite pytest is
permitted, not required (AP_DEFECTS D-03) — if you run it, you MUST still deselect the same
two nodeids (or equivalent `-k` exclusion). Frontend: you MAY run focused vitest on
`diagnostic-target-fetch`, `diagnostic-target-runtime`, `diagnostic-egress`,
`openai-compatible`, `provider-logging`, `ai-play-diagnostic` (including `.worker.test`),
and `app/api/ai/move/route.test.ts`; `npm run build` is forbidden. Quote whatever you run
VERBATIM.

⛔ Never ambient `python`, `python3`, or `poetry run`. ⛔ Never a second `-q`.
⛔ Never set `PYTHON_DOTENV_DISABLED=1` on any command you type. That is the Django/test
route prohibition. It is not a ban on reading or collecting files that mention the name.
...
· ⛔ Do not stop solely because `_migration_process` exists or assigns PYTHON_DOTENV_DISABLED
  in source. That conflict is already dispositioned (F01) and excluded from required execution.
· ⛔ Do not stop after confirming the deselect grant. Confirmation without the §6 report is
  non-performance.
```
...
Exclusions: K1 live NIM; slice 8 probe/history; live egress activation; whole-closure R4/R5;
  ...
  except "untouched in diff"; F-V/F-W historical migration-rescue execution (static caller check only)
Threat model: §2 (echo any delta you measured)
Source records: OWASP ASVS 5.0; MITRE CWE (refresh retrieval dates from the LOCAL AP registry
  only — network NONE, so do not claim a live external refresh; CWE-918 is the SSRF signal)
Findings: every finding in the Security Finding Record shape (PROMPT_CONTRACTS.md:1772-1817),
  including rejected-false-positive results. IDs APMC-S7-IA-Fnn starting at F02.
  F01 is already used (orchestration, accepted). If you have zero product findings, say so
  explicitly; do not mint a duplicate F01.
...
Orchestration critique: none | <MEASURED and LEAD — Did you run the deselected pytest in this
  session? Is C12 the wrong question? Did C7 ai-context weight match the asset? Did anyone
  treat fake Launch as seam proof?>
