...
Session 24 attempted this same 7-IA and **BLOCKED** before any product dynamic evidence, because its required four-module pytest command would have *executed* two historical migration tests whose helper sets `PYTHON_DOTENV_DISABLED` in a subprocess. That was an **orchestration grant conflict**, not a demonstrated product vulnerability. This prompt **resolves the grant**. You still audit C1–C12 independently. You do **not** inherit session 24’s “not accepted” table — those rows mean “evidence not collected,” not twelve defects.
...
Session 24’s report (`24_report_00.md`) is authorized reading **only** as the orchestration-blocker
record (APMC-S7-IA-F01). Do not copy its C1–C12 verdicts.
...
APMC-S7-IA-F01 (session 24, confirmed, info, Orchestrator-accepted): required four-module
  ...
  those two tests. ⛔ Do not BLOCK again because the helper still exists in source.
...
Focused dynamic evidence (required). Run this exact command from `backend/`. The two
`--deselect` nodeids are historical slice-4 migration-rescue tests (F-V / F-W). They are
...
Orchestration critique: none | <MEASURED and LEAD — Did the deselect grant let you finish C1–C12?
  Is C12 the wrong question? Did C7 ai-context weight match the asset? Did anyone treat fake
  Launch as seam proof?>
