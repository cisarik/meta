...
- **Performance**: equity is computed only per *certified* candidate (post-`evaluate_scoring_move`, ≤ `max_unique_placements` = 25 000 worst case, typically hundreds), each O(rack_size) = O(7) with memo hits. Searcher overhead is negligible (< 2 ms/ply).
- **Public signature of `find_ranked_scoring_moves`** is **unchanged** — `variant` and `tile_points` parameters already carry everything the profile needs; `bag_count` already exists for the endgame branch. Callers (`selfplay._ranked_search`, `services._probe_ai_ranked_candidates`, `position_sets._ranked_on_game`, `diagnostics`) need zero call-site changes.
