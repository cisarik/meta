...
Task identity: CONTEXTDECK-UI-UX-REFINEMENT-S3-S4-S5
Native planning mode: not-used
Delivery route: manual COOPERATOR delivery to the exact current Worker session
Continuity anchor: terminal implementation report `01_report_02.md` for Worker
exchange `03` (S1+S2+S9) and the accepted plan report `01_report_01.md` for
Worker exchange `02` — all produced in this same session
Authority renewal: prior implementation authority expired at the exchange-03
...
presentation slices with exact file allowlists, a fixed visual-language and
accessibility rule set, one confirmed new string, and the full registered suite
plus the binding-set invariant as gates); no open design question and no live
host mutation.
...
# ContextDeck — implement S3, S4, and S5 of the accepted UI/UX plan
...
three accepted slices in order: S3 (Stav surface), S4 (Farby surface), S5
(Aplikácie surface). Apply the accepted plan exactly, validate each slice,
...
exchange-03 terminal report. Retained context, the plan text, and this
...
Reconciled predecessor exchange: S1+S2+S9 implementation-PASS on
`e22c9be1bfa3ad31ed7155f3af5f3aa8bd30a2a0`
...
COOPERATOR decision recorded for this exchange: keep the one proposed new
profile-removal confirmation string in S5 exactly as the plan proposed it. No
other new or changed user-visible string is authorized in this exchange.

## Implementation Authority Record

Implementation authority: explicit
Exact baseline: `e22c9be1bfa3ad31ed7155f3af5f3aa8bd30a2a0`
Changed-path allowlist: `ui/OverviewPage.qml`; `ui/ZoneHero.qml`;
`ui/ColorsPage.qml`; `ui/LightingPresetEditor.qml`; `ui/ApplicationsPage.qml`
Per-slice allowlists: S3 = `ui/OverviewPage.qml`, `ui/ZoneHero.qml`; S4 =
`ui/ColorsPage.qml`, `ui/LightingPresetEditor.qml`; S5 =
...
`app.*` binding, property name, signal, and behavior; the only authorized
string change is the confirmed S5 profile-removal confirmation; no schema,
IPC, broker, packaging, dependency, license, or non-allowlisted path change
...
Exact baseline HEAD: `e22c9be1bfa3ad31ed7155f3af5f3aa8bd30a2a0`
First slice commit parent must be: `e22c9be1bfa3ad31ed7155f3af5f3aa8bd30a2a0`
...
Report destination:
`projects/contextdesk/00/07-ui-ux-refinement/01_report_03.md`
...
5. Verify the trace directory is real and contains `00_handout.md`,
   ...
   `01_report_01.md`, `01_implementation_02.md`, and `01_report_02.md`; the
   report destination `01_report_03.md` must be absent before the write. Do not
   read or copy `01_report_00.md`: it is the retired frozen artifact and
   contains non-public local paths.
6. Read the accepted plan report `01_report_01.md` completely, especially
   sections 3 (VL rules, per-surface list, the confirmed string, A
   requirements), 4 (slices S3/S4/S5 and allowlists), and 7 (acceptance control
   matrix); confirm the three slice scopes match this prompt.
7. Read all five allowlisted QML files completely before editing; capture the
   baseline `app.*` binding set from `ui/` first.
...
- VL-1 page root spacing stays `Kirigami.Units.largeSpacing`; intra-group
  `smallSpacing`.
- VL-2 each section page opens with a `Kirigami.Heading` level 2 (Stav keeps
  hero-first, no redundant heading).
...
- VL-7 typography uses `Kirigami.Theme.defaultFont/smallFont` and relative
  sizes (keep the Overview `*1.12`).
- VL-8 semantic `Kirigami.Theme.*` colors only; no new literal colors
  (existing zone/effect constants stay).
- VL-9 replace fixed pixel sizes that harm legibility with
  `Kirigami.Units`/`gridUnit` where layout intent is preserved; keep the
  ZoneHero strip metaphor conservative.
- VL-10 every interactive control is keyboard-reachable in reading order with
  a visible focus indicator.
- A-1 add `Accessible.name` to every interactive control lacking one (using
  existing mixed-language strings verbatim).
- A-2 add `Accessible.role` to custom interactive items (swatches →
  `Accessible.Button`).
- A-3 add `Accessible.description` where purpose needs context.
- A-4 associate labels with controls by converting ad-hoc label/control rows to
  `Kirigami.FormLayout` + `Kirigami.FormData.label` (presentation-only;
  Diagnostics already does this).
- A-5 `activeFocusOnTab: true` + visible focus indicator for custom swatches,
  with `Keys.onReturnPressed`/`onSpacePressed` and `Accessible.onPressAction`
...
- A-8 give `ZoneHero` an accessible text summary.

## S3 scope (accepted plan section 3, exact)

Changed-path allowlist: `ui/OverviewPage.qml`, `ui/ZoneHero.qml`.

- Consistent notice/status semantics (VL-5, VL-6).
- Accessible summary for `ZoneHero` (badge + zone names) and a role on the
  hero.
- Make custom hero content focusable only if it is interactive (it is not; do
  not add focusability to non-interactive visuals).
- Keep `app.temporaryOverrideActive`, `app.heroKind`, `app.heroBadge`,
  `app.heroZones`, `app.zoneNames`, `app.workspaceSummary`,
  `app.statusSummary`, `app.restoreAutomatic`, `app.restoreDeviceDefault`, and
  `app.lightsOff` unchanged.
- Strings unchanged.

## S4 scope (accepted plan section 3, exact)

Changed-path allowlist: `ui/ColorsPage.qml`, `ui/LightingPresetEditor.qml`.

- Heading/intro/status per VL-2, VL-3, VL-5.
- Label association for editor rows (A-4).
- Accessible names, roles, and descriptions on the preset `ComboBox`, speed
  `Slider`, breathing swatch, zone swatches, hex toggle, gradient swatches, and
  "Použiť gradient" (A-1..A-3).
- Focusable custom swatches with keyboard activation (A-5).
- Preserve the editor's public property interface exactly (`profileId`,
  `currentMode`, `zones`, `applicationLevel`, `speedPercent`, `breathingHex`,
  `workspaceLayoutActive`, `zoneSlots`, `effectiveMode`, `readyToSave`) because
  S5 nests the same editor.
- Preserve every `app.*` invocation (`applyMode` →
  `setGlobalLightingMode`/`setApplicationLightingMode`, `applyZone`,
  `applyGradient`, `applySpeed`, `applyBreathingColor`, `openZonePicker`,
  `openGradientPicker`, `openBreathingPicker`, `previewGradient`).
- Strings unchanged (keep "Rýchlosť animácie", "Farba dýchania", "Pick color
  for …", etc.).
...
- Heading/intro/status per VL-2, VL-3, VL-5.
- Empty-state notice when `app.inventory` is empty and when `app.profiles` is
  empty (VL-6, `Information`).
- Confirmation for profile removal via a new QML `Controls.Dialog` (the
  COOPERATOR-confirmed string):
  ...
  This is the only authorized string change in the whole exchange.
- Label association for assignment rows (A-4).
- Keep every `app.*` call and every `modelData` field unchanged.
...
- all five allowlisted QML files in full;
...
- All 83 `app.*` binding names remain unchanged; no behavior, property, signal,
  or user-visible string changes except the one confirmed S5 string.
...
Inspection and provenance: required — read all five files before editing and
capture the baseline `app.*` binding set and the baseline string-literal set
from `ui/` before the first edit
...
1. Capture baseline evidence before edits: the sorted unique `app.*` name set
   from `ui/*.qml`; the string-literal inventory of the five allowlisted files
   (for the drift check); and a clean `git status`.
2. Apply S3 only; validate; commit; push; verify.
3. Apply S4 only; validate; commit; push; verify.
4. Apply S5 only; validate; commit; push; verify.
5. Inspect each commit's changed paths against its per-slice allowlist, and the
   cumulative string-literal drift against the one confirmed S5 string.
...
property name, a signal, or an unauthorized string, stop with `PARTIAL` and
name the exact obstacle; do not widen the allowlist.
...
2. Create exactly one normal commit with the exact subject:
   - S3: `ui: refine Stav overview and zone hero`
   - S4: `ui: refine Farby lighting surface and preset editor`
   - S5: `ui: refine Aplikácie application profiles surface`
...
- tests and validation, including the full-suite result per slice, the
  ...
  result against the one confirmed string;
- commit and push result per slice plus direct public-ref verification;
- accessibility evidence: the added names, roles, descriptions, focusability,
  and label associations per surface, with the exact QML locations;
