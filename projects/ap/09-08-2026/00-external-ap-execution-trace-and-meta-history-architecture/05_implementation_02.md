# Worker 5 — Current-Session AP-Native External Trace Contract Implementation, Exchange 02
...
Continuity anchor: continue the exact same healthy Worker 5 session that returned the terminal `05_report.md` for exchange `01`. That report expired the prior authority and truthfully stopped `BLOCKED` before mutation at exact AP baseline `1b0774117e1de7ecabddc7f08d15dbaf3068b09b`. The repository, objective, implementation route, semantic decisions, risk class, allowlist, and independence requirement are unchanged. ORCHESTRATOR reconciliation below resolves the only launch blocker. Retained context supplies continuity only; this complete prompt is the new authority grant.

Native planning mode: `not-used`

Worker session profile: `Current AP Protocol Implementation Worker`

Phase: `Implementation`

Task identity: `AP-EXTERNAL-TRACE-CONTRACT-IMPLEMENT-W05-X02`
...
Authorized implementation stages: exact current-session re-gating; accepted outer-environment containment; complete required reading; baseline validation; allowlisted implementation; focused and full validation; exact diff and semantic-owner review; one local non-amended candidate commit; post-commit validation; terminal report.
...
26. Worker 5 exchange `01` is a truthful terminal `BLOCKED` exchange. It created no AP or Meta mutation and is not rewritten, erased, or converted into PASS by this continuation.
27. Continuing the same Worker 5 session is proportionate because its context is healthy, it obeyed the stop gate, the task and route assumptions remain unchanged, and independent evidence is not required from the implementation Worker.
...
#### Worker 5 exchange 01 baseline reconciliation

Worker 5 exchange `01` ran the required unchanged baseline suite and observed exit `1`, `passed: 90`, `failed: 1`. The sole failure was `project exec preserves symlinked virtualenv launch semantics`. The captured output contained the inherited marker name `VIRTUAL_ENV_DISABLE_PROMPT`; the test then rejected the broad substring `VIRTUAL_ENV`. No traceback, repository mutation, or candidate existed.

The ORCHESTRATOR independently reproduced the exact mechanism at immutable AP baseline `1b0774117e1de7ecabddc7f08d15dbaf3068b09b` in sequential isolated temporary roots without inspecting or recording any inherited value:

```text
env -u VIRTUAL_ENV_DISABLE_PROMPT sh tests/ap_tool_tests.sh
=> exit 0; passed: 91; failed: 0

VIRTUAL_ENV_DISABLE_PROMPT=<synthetic-test-only> sh tests/ap_tool_tests.sh
=> exit 1; passed: 90; failed: 1
=> sole failure: project exec preserves symlinked virtualenv launch semantics
```

Classification: this is a pre-existing outer-environment/test-output interaction, not an AP baseline regression and not evidence that the symlinked virtualenv launch or sanitized child environment is incorrect. The `ap` runner safely reports inherited contamination marker names; the failing assertion is broader than the child-environment evidence it intends to check.

Accepted containment for this exchange is exactly:

```text
env -u VIRTUAL_ENV_DISABLE_PROMPT sh tests/ap_tool_tests.sh
```

Use that form for the baseline suite and every later full-suite invocation in this exchange. Unsetting this one inherited prompt-control marker only for the test command does not weaken the required exit/count gate and does not authorize an AP source or harness repair. Do not edit `ap` or `tests/ap_tool_tests.sh` merely to suppress, rename, or special-case this marker. The allowlisted test file may still change only for the RF-19 enforcement required by this prompt. Do not reproduce the synthetic failing run. Do not inspect or print the marker's value. If the contained suite has any non-zero exit, traceback, count other than `91/0` at baseline, or different causal failure, stop `BLOCKED` as new evidence.
...
5. Verify `05_implementation.md`, `05_report.md`, `05_implementation_02.md`, and `05_report_02.md` are not present anywhere inside the AP worktree. If any is present, stop; do not absorb, move, delete, or commit it.
6. Resolve trusted system binaries without `cursor`, `code`, `xdg-open`, GUI, AppImage, or IDE-integrated wrappers.
7. Do not rerun the known-failing uncontained suite. Run the accepted contained baseline suite before editing:

```text
env -u VIRTUAL_ENV_DISABLE_PROMPT sh tests/ap_tool_tests.sh
```

Require exit `0`, `passed: 91`, and `failed: 0`. Record the command form and classify exchange `01`'s `90/1` result under the accepted reconciliation above. Preserve the first causal failure. A non-zero exit, different count, or traceback forbids implementation PASS.
...
4. Run focused checks during editing and the contained full suite after the coherent change.
...
10. After commit, rerun the contained full suite from the candidate, verify clean status, exact parent, tree, subject, changed paths, and local/public divergence.
...
Use the accepted contained full-suite command before edits, after the coherent worktree change, and after the candidate commit. Do not substitute an alias, wrapper, broader environment rewrite, or uncontained full-suite command. Use safe quoting appropriate to the active shell. Public readback must remain credential-free and non-interactive. Preserve exit codes and the first causal failure. A traceback, non-zero required check, missing negative test, dirty post-commit status, wrong staged set, or public-main movement forbids `PASS`.
...
2. The accepted contained baseline suite passed `91/0` before edits, and exchange `01`'s uncontained `90/1` result is reported as a reconciled pre-existing environment/test-output interaction rather than hidden.
...
12. The contained full suite exits `0`, `failed: 0`, with passed count greater than `91` before and after commit.
...
Do not create, copy, edit, stage, or commit `05_implementation.md`, `05_report.md`, `05_implementation_02.md`, `05_report_02.md`, or any Meta file.

The current exchange-02 prompt is delivered externally. After your terminal report expires your authority, Michal may use separately held archival authority to add the exact `05_implementation_02.md` and exact `05_report_02.md` together to Meta. That archival commit is not part of the AP candidate, does not grant AP authority, and need not be referenced by its own report. The already archived exchange-01 pair remains truthful and unchanged.
...
- this is not the exact same healthy Worker 5 session that returned exchange `01`'s `05_report.md`;
...
- any Worker 5 prompt/report archive artifact is present inside the AP worktree;
- the accepted contained baseline suite does not exit `0` with `91 passed` and `0 failed`;
- the known marker interaction would require an AP source/harness repair, a broader environment rewrite, inspection of an environment value, or any containment beyond the exact authorized command;
...
Do not repair the reconciled pre-existing environment/test-output interaction. Do not weaken a gate. Do not continue into acceptance or publication.
...
2. route, current-session continuity, complete authority renewal, and capability observations without inventing model/reasoning facts;
3. exact baseline and public readback;
4. the exchange-01 blocker, ORCHESTRATOR reconciliation, accepted containment command, and exact contained baseline suite result;
...
Resolved Execution Issues / Near-Misses: exchange 01 stopped before mutation on the inherited VIRTUAL_ENV_DISABLE_PROMPT marker interaction; exchange 02 used the exact ORCHESTRATOR-authorized contained full-suite command; <any additional exact list or none>
Pre-Existing Failure Classification: Worker 4 Meta-only exchanges remained BLOCKED before mutation; Worker 5 exchange 01 remained BLOCKED before mutation because of the reconciled inherited marker/test-output interaction; no prior AP or Meta implementation candidate existed
```
