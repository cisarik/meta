# Kronika one product — S3 read-only NUC capture preflight (renewed)
...
This must be a genuinely fresh session launched from an environment that
already exports the three `FRAMENEST_NUC_SSH_*` names. This is a read-only
preflight: inspect and report, never mutate the NUC. `PASS` recommends a
separately authorized host setup/deployment grant; it never authorizes it.

This grant renews the previous exchange after its transport precondition
failure (`05_report_00.md`, status BLOCKED: the three transport names were
unset in the Worker environment; no remote command was sent).
...
- The three `FRAMENEST_NUC_SSH_*` names are exported into the environment that
  launches this Worker session (values never appear in chat, prompts or
  reports).
