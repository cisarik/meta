# Kronika one product — S3 read-only NUC capture preflight
...
This must be a genuinely fresh session. This is a read-only preflight: inspect
and report, never mutate the NUC. `PASS` recommends a separately authorized host
setup/deployment grant; it never authorizes it.

## Cooperator preconditions (outside the Worker)

- The `FRAMENEST_NUC_SSH_*` transport variables are exported into this Worker's
  environment (names only are ever checked; values are never printed or stored).
...
1. **Gate and privilege.** `scripts/operator/network/framenest_nuc_worker_gate.fish
   --probe` prints `ssh-agent: ready`, exit 0. All three `FRAMENEST_NUC_SSH_*`
   variables are set (names only). Remote `sudo -n true` exit 0; if it fails
   with a password prompt, report expected lifecycle state and stop.
2. **Binary paths and versions.** For each hardcoded path: existence and
   ...
   (no launch, no flags). Report exact versions and whether the path is a
...
9. **Display tooling.** `Xvfb`, `xauth`, `mcookie` present and their versions
   (already in item 2); confirm the runner's and Xvfb unit assumptions
...
Commands: the gate `--probe`; bounded BatchMode remote read-only commands;
`sudo -n` read-only probes; `ss`, `pgrep`, `getent`, `test`, `ls`, `stat`,
`readlink`, `command -v`, `systemctl --version`, read-only `systemctl status`;
and the native file writer for the report only.
...
Stop and report on: a failed gate or privilege probe; any check that would
require mutation; sensitive output; an unexpected host state that makes the
remaining checks unsafe; or any instruction conflict. Preserve the first causal
failure. A read-only preflight never repairs anything.

## Completion and report contract

`PASS` means every required check above was observed read-only and the report
states exact values, mismatches, unknowns and a recommendation for the bounded
host setup/deployment grant. Use `PARTIAL` when a check could not be completed
or a material unknown remains; `BLOCKED` when the preflight cannot proceed
safely. No mutation occurred; no deployment claim is made. Use
...
prompt's three coordinates exactly once. Include the compact core; a per-check
observation table with exact sanitized values and exit codes; the mismatch list
against the unit/template assumptions; unknowns; the sudo lifecycle evidence
including the terminal release; one smallest next step; `Report justification:
...
Downloadable prompt filename: 05_preflight_00.md
